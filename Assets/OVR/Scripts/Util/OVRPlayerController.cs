...
public class OVRPlayerController : MonoBehaviour
{
	...
	public virtual void UpdateMovement()
	{
		...
		if (OVRGamepadController.GPC_GetButton(OVRGamepadController.Button.Left) || (Input.GetAxis("Mouse X")>0.0f))
		{
			moveLeft = true;
			dpad_move   = true;
			
		}
		if (OVRGamepadController.GPC_GetButton(OVRGamepadController.Button.Right) || (Input.GetAxis("Mouse X")<0.0f))
		{
			moveRight  = true;
			dpad_move = true;
		}

		if (OVRGamepadController.GPC_GetButtonDown (OVRGamepadController.Button.X))
		{
			//BallMovement.Instance.KickBall ();
		}
		...
		/*if (moveForward)
			MoveThrottle += ort * (transform.lossyScale.z * moveInfluence * Vector3.forward) * 2.0f;
		if (moveBack)
			MoveThrottle += ort * (transform.lossyScale.z * moveInfluence * Vector3.back) * 3.0f;*/
		if (moveLeft)
			MoveThrottle += ort * (transform.lossyScale.x * moveInfluence * BackAndSideDampen * Vector3.left) * 10.5f;
		if (moveRight)
			MoveThrottle += ort * (transform.lossyScale.x * moveInfluence * BackAndSideDampen * Vector3.right) * 10.5f;
